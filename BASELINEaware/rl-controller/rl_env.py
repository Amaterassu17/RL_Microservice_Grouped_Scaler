...
class RLEnvironment:
    ...
    def __init__(self, app_name='my-app', app_namespace='my-app-ns', mpa_name='my-mpa', mpa_namespace='my-mpa-ns'):
        ...
        ret = self.corev1.list_pod_for_all_namespaces(watch=False)
        for i in ret.items:
        ...
        if os.getenv("PROM_HOST"):
            print('PROM_HOST is set to:', os.getenv("PROM_HOST"))
            user_specified_prom_host = os.getenv("PROM_HOST")
            if not user_specified_prom_host in PROM_URL:
                #MODIFIED I ASSUME IT HAS ALREADY GOT HTTPS
                PROM_URL = user_specified_prom_host
        ...
        #     exit()
        # else:
        #     PROM_TOKEN = os.getenv("PROM_TOKEN")clear
        print('PROM_URL:', PROM_URL)
        print('PROM_TOKEN:', PROM_TOKEN)
        self.prom_client = PromCrawler(prom_address=PROM_URL, prom_token=PROM_TOKEN)

        #testing
        PROM_URL_USERSPACE=PROM_URL
        PROM_TOKEN_USERSPACE=PROM_TOKEN
    ...
    def observe_states(self):
        ...
        for resource in self.controlled_resources:
            ...
            if resource.lower() == "cpu":
                ...
                #resource_query = "irate(container_memory_usage_bytes{%s}[5m])"
                resource_query = "container_memory_usage_bytes{%s}"
            elif resource.lower() == "blkio":
                resource_query = "container_blkio_device_usage_total{%s}"
            ...
            for container_query in container_queries:
                if(resource.lower() == "ingress" or resource.lower() == "egress"):
                    query_index = namespace_query
                else:
        ...
        #print(traces[self.app_name]['blkio'])
    ...
    def get_target_containers(self):
        # print('Getting target containers')
        # print('App name:', self.app_name)
        # print('App namespace:', self.app_namespace)
        if(self.app_name == 'php-apache'):
            target_pods = self.corev1.list_namespaced_pod(namespace=self.app_namespace, label_selector="run=" + self.app_name)
        else:
            target_pods = self.corev1.list_namespaced_pod(namespace=self.app_namespace, label_selector="app=" + self.app_name)

        # print('Target pods:', target_pods)
        target_containers = []
    ...
    def print_info(self):
        ...
        if ('cpu-limit' in self.states) and ('memory-limit' in self.states):
            print('Initial allocation: CPU limit =', str(self.states['cpu-limit'])+'m', 'Memory limit:', self.states['memory-limit'], 'MiB')

        if('cpu_util' in self.states):
            print('Avg CPU utilization: {:.3f}'.format(self.states['cpu_util']))

        if('memory_util' in self.states):
            print('Avg memory utilization: {:.3f}'.format(self.states['memory_util']))

        if('disk_io_usage' in self.states):
            print('Avg disk usage: {:.3f}'.format(self.states['disk_io_usage']))


        if('ingress_rate' in self.states):
            print('Avg ingress rate: {:.3f}'.format(self.states['ingress_rate']))

        if('egress_rate' in self.states):
            print('Avg egress rate: {:.3f}'.format(self.states['egress_rate']))

        if('file_discovery_rate' in self.states):
            print('Avg file discovery rate: {:.3f}'.format(self.states['file_discovery_rate']))

        if('rate' in self.states):
            print('Avg rate: {:.3f}'.format(self.states['rate']))

        if('processing_rate' in self.states):
            print('Avg processing rate: {:.3f}'.format(self.states['processing_rate']))

        if('ingestion_rate' in self.states):
            print('Avg ingestion rate: {:.3f}'.format(self.states['ingestion_rate']))
        
        if('latency' in self.states):
            print('Avg latency: {:.3f}'.format(self.states['latency']))

        if('num_replicas' in self.states):
